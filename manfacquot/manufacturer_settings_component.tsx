// Manufacturer Settings Component - Insert this in index.tsx before ManufacturerDashboard
...
const ManufacturerSettingsPage = () => {
    ...
    return (
        <div>
            ...
            {/* Tabs */}
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', overflowX: 'auto', borderBottom: '1px solid var(--border-color)' }}>
                {tabs.map(tab => (
                    <button
                        key={tab.id}
                        ...
                        style={{
                            ...
                            border: 'none',
                            borderBottom: `2px solid ${activeTab === tab.id ? 'var(--neon-cyan)' : 'transparent'}`,
            ...
            {/* Tab Content */}
            <div style={{ background: 'var(--bg-panel)', padding: '24px', borderRadius: '8px', minHeight: '400px' }}>
                ...
                {activeTab === 'processes' && (
                    <div>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '24px' }}>Select processes your shop can perform</p>

                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
                            {['milling_3_axis', 'milling_5_axis', 'turning', 'turning_live_tooling', 'sheet_laser_cut', 'sheet_waterjet', 'sheet_punch', 'wire_edm'].map(process => (
                                <label key={process} style={{ display: 'flex', alignItems: 'center', padding: '12px', background: 'var(--bg-panel)', borderRadius: '6px', cursor: 'pointer' }}>
                                    <input
                                        type="checkbox"
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (e.target.checked) {
                                                updateSetting(['processes'], [...processes, process]);
                                            } else {
                                                updateSetting(['processes'], processes.filter(p => p !== process));
                                            }
                                        }}
                                        style={{ marginRight: '8px' }}
                                    />
                                    <span style={{ color: '#CBD5E1' }}>{process.replace(/_/g, ' ').toUpperCase()}</span>
                                </label>
                            ))}
                        </div>
                    </div>
                ...
                {activeTab === 'secondary' && (
                    <div>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '24px' }}>Configure secondary process capabilities (future expansion)</p>
                        <div style={{ padding: '40px', textAlign: 'center', color: '#64748B' }}>
                            <p>Secondary operations configuration coming soon.</p>
                            <p style={{ fontSize: '12px', marginTop: '8px' }}>Will include: Anodizing, Plating, Heat Treatment, Powder Coating, etc.</p>
                        </div>
                    </div>
                )}
