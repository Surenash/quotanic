...
export const SignupRoleSelector = ({ navigate }: { navigate: (page: string, params?: any) => void }) => (
    <div style={styles.loginPage}><div style={styles.loginContainer}><h2 style={styles.loginTitle}>Join Quotanic</h2><p style={styles.loginSubtitle}>Select your account type to get started.</p><div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '32px' }}><CtaButton text="Sign Up as a Customer" primary onClick={() => navigate('signup-customer')} /><CtaButton text="Sign Up as a Manufacturer" onClick={() => navigate('signup-manufacturer')} /></div><div style={{ textAlign: 'center', marginTop: '24px' }}><p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Already have an account? <a href="#" style={{ ...styles.loginLink, fontSize: '14px' }} onClick={(e) => { e.preventDefault(); navigate('login'); }}>Log In</a></p></div></div></div>
);