...
class CustomerManager(BaseUserManager):
    ...
    def create_user(self, email, password=None, **extra_fields):
        ...
        return self.model(email=email, role=UserRole.CUSTOMER, **extra_fields).save() # This might need adjustment to match BaseUserManager signature properly, usually just defer to creating a user with role preset.
...
class Manufacturer(models.Model):
    ...
    about = models.TextField(blank=True, null=True, help_text="Short bio or description of the manufacturer.")
    ...
    # Store image URLs/Base64 (temporary solution until S3 is set up) or external links
    logo_url = models.TextField(blank=True, null=True)
    background_url = models.TextField(blank=True, null=True)
    
    # Store portfolio items as valid JSON: [{"id": 1, "title": "Part A", "url": "...", "type": "image"}]
    portfolio_data = models.JSONField(blank=True, null=True, default=list)
